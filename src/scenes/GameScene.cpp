...
Scenes::GameScene::GameScene(): Scene(std::make_shared<Camera2D>()),
                                po_levels_(std::make_unique<std::vector<CoreLogic::DataProcessing::Level>>(std::initializer_list<CoreLogic::DataProcessing::Level>{
                                        CoreLogic::DataProcessing::Level(std::make_unique<std::vector<std::string>>(std::initializer_list<std::string>{"assets/data/hive_DESI_Level0-Greyboxing_2024-06-17.tmj", "assets/data/hive_DESI_Level3-Greyboxing_2024-06-17.tmj"}), 0, CoreLogic::DataProcessing::LevelState::Default),
...
void Scenes::GameScene::update()
{
    ...
    camera.target = (Vector2){ (float)round(camera.target.x), (float)round(camera.target.y) };
